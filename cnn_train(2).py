...
def cnn_model_fn(features):
    ...
    conv1 = tf.layers.conv2d(inputs=input_layer, filters=64, kernel_size=[3, 3],
                             ...
                             bias_initializer=tf.contrib.layers.xavier_initializer(), padding="same", activation=tf.nn.relu)
    ...
    fc1 = tf.layers.dense(inputs=pool3_flat, units=1024,
                          kernel_initializer=tf.contrib.layers.xavier_initializer(),
                          bias_initializer=tf.contrib.layers.xavier_initializer(),activation=tf.nn.relu, name='fc1')

    fc1 = tf.layers.dropout(inputs=fc1, rate=dropout_il) if cnn_mode == 'train' else fc1  # Dropout input layer

    fc2 = tf.layers.dense(inputs=fc1, units=1024, kernel_initializer=tf.contrib.layers.xavier_initializer(),
    ...
    fc4 = tf.layers.dense(inputs=fc3_bn, units=10, kernel_initializer=tf.contrib.layers.xavier_initializer(),
                          bias_initializer=tf.contrib.layers.xavier_initializer())
...
fc1_weights = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, 'fc1')[0]
fc1_bias = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, 'fc1/')[1]
...
print('Fully connected Layer 1 weights', sess.run(fc1_weights))
print('Fully connected layer 1 weights shape', sess.run(fc1_weights).shape)
print('Fully connected Layer 1 bias', sess.run(fc1_bias))
print('Fully connected Layer 1 bias', sess.run(fc1_bias).shape)
